...
- Dev box: `<dev-box>` (SSH user: `ubuntu`, 32 cores, 123GB RAM)
...
ssh ubuntu@<dev-box> "sudo systemctl stop pillar-agent && sudo cp /tmp/pillar-build/target/release/agent /usr/local/bin/pillar-agent && sudo systemctl start pillar-agent"
```
