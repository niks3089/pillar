...
/// Returns true for sudo/PAM noise lines that should be silently dropped.
/// These are generated by `sudo systemctl` calls from the agent and attributed
/// to the agent's systemd unit because the child process shares its cgroup.
fn is_sudo_noise(message: &str) -> bool {
    message.starts_with("pam_unix(sudo:")
        || (message.contains("; USER=root ; COMMAND=") && message.contains("PWD="))
}
...
fn parse_journal_line(line: &str) -> Option<LogEntry> {
    ...
    if message.is_empty() || is_sudo_noise(&message) {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn parse_sudo_noise_returns_none() {
        let line = r#"{"MESSAGE":"pam_unix(sudo:session): session opened for user root(uid=0) by (uid=994)","PRIORITY":"6","__REALTIME_TIMESTAMP":"1700000000000000","_SYSTEMD_UNIT":"pillar-agent.service"}"#;
        assert!(parse_journal_line(line).is_none());
    }
